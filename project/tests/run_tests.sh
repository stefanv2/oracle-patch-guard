...
MOCK_SQLPATCH_BEFORE='SQLPATCH|37960098|APPLY|WITH ERRORS|20200101120000000000;SQLPATCH|39034528|APPLY|SUCCESS|20260901090000000000;SQLPATCH|38906621|APPLY|SUCCESS|20260901090100000000;SQLPATCH|39472050|ROLLBACK|SUCCESS|20260901090200000000;SQLPATCH|39222882|ROLLBACK|SUCCESS|20260901090300000000'
EOF
precheck PA-FUTURE; rc=$?
grep -Fq 'READY|REGISTRY_SQLPATCH_READINESS|' "$RUN_ROOT/PA-FUTURE/precheck_summary.psv" || rc=99
grep -Fq 'READY|reason=target_not_installed|patch_id=39472050|action=ROLLBACK|status=SUCCESS|' "$RUN_ROOT/PA-FUTURE/sqlpatch_readiness_DB1.psv" || rc=96
grep -Fq 'READY|reason=target_not_installed|patch_id=39222882|action=ROLLBACK|status=SUCCESS|' "$RUN_ROOT/PA-FUTURE/sqlpatch_readiness_DB1.psv" || rc=95
...
record_precheck 'succesvol teruggedraaide targetpatches laten toekomstige targetcycle toe' 10 "$rc" "$CASE_DIR/PA-FUTURE.out"

setup_case planupgradeafterrollback
cat >>"$FIXTURE_ENV" <<'EOF'
MOCK_INVENTORY_BEFORE_ROWS='Patch 39034528 : applied;Patch 38906621 : applied'
MOCK_SQLPATCH_BEFORE='SQLPATCH|39034528|APPLY|SUCCESS|20260901090000000000;SQLPATCH|38906621|APPLY|SUCCESS|20260901090100000000;SQLPATCH|39472050|ROLLBACK|SUCCESS|20260901090200000000;SQLPATCH|39222882|ROLLBACK|SUCCESS|20260901090300000000'
EOF
guard plan --non-interactive --target-oracle-home "$HOME_DIR" --run-id PA-UPGRADE 39472050 39222882 JUL2026 12.2.0.1.52 p6880880_190000_Linux-x86-64.zip >"$CASE_DIR/PA-UPGRADE.out" 2>&1; rc=$?
[[ -f "$RUN_ROOT/PA-UPGRADE/proposed_runbook.sh" ]] || rc=99
grep -Fq '|TARGET_PATCHLEVEL_ALREADY_APPLIED|' "$RUN_ROOT/PA-UPGRADE/findings.psv" && rc=98
record 'PLAN staat normale upgrade toe na succesvolle target-rollback' 0 "$rc" "$CASE_DIR/PA-UPGRADE.out"
